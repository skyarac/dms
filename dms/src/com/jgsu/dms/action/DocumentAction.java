...
import com.jgsu.dms.service.IDepartmentService;
import com.jgsu.dms.service.IDocumentService;
...
public class DocumentAction extends ActionSupport implements ModelDriven<Document>{
	...
	public String listReceive() {
		...
		if(isRead==null||isRead==0) {
			ActionContext.getContext().put("pages", 
					documentService.findNotReadDocument(con, depId));
		} else {
			ActionContext.getContext().put("pages",
					documentService.findReadDocument(con, depId));
		}
		return SUCCESS;
	}
	...
	public String listSend() {
		...
		ActionContext.getContext().put("pages",
				documentService.findSendDocument(u.getId()));
		return SUCCESS;
	}
